--------------------------------- MODULE RaftWithReconfigBroken_Bounded --------------------------------
\* 
...
Next ==
    ...
    \* \/ \E i,j \in Server : AppendOplog(i, j) /\ StateConstraint'
    \* \/ \E i,j \in Server : RollbackOplog(i, j)
    \/ \E i \in Server : \E ayeVoters \in (SUBSET(Server) \ {Server}) : BecomePrimary(i, ayeVoters) /\ StateConstraint'
    \* \/ \E i \in Server : ClientWrite(i)
    \* \/ \E leader \in Server : \E ack \in SUBSET Server : AdvanceCommitPoint(leader, ack)
    \/ \E i \in Server : \E newConfig \in SUBSET(Server) : Reconfig(i, newConfig) /\ StateConstraint'
    \* \/ \E i,j \in Server : UpdateTermThroughHeartbeat(i, j)
