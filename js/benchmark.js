...
async function testStateGraphBenchmark(testId, stateGraph, parsedSpec, specPath, constvals, spec) {
    ...
    for (let i = 0; i < NUM_ITERATIONS; i++) {
        ...
        reachableObj = interp.computeReachableStates(parsedSpec, constvals, undefined, spec);
    }

    // Take average duration.
    ...
    function fetchTestSpec(test, onCompleteFn) {
        ...
        return Promise.all([fetchGraph,fetchSpec]).then(function([specStateGraph, specText]){
            ...
            spec.parse().then(function () {
                ...
                return testStateGraphBenchmark(test["spec"], specStateGraph, parsedSpec, specPath, test["constvals"], spec)
